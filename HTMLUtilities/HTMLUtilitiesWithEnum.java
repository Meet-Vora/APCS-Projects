 ...
 */
public class HTMLUtilitiesWithEnum {
	...
	public String[] tokenizeHTMLString(String str) {
		...
		while (counter < str.length()) {
			...
			// check if it's the beginning of the comment
			...
			else if(str.indexOf("<pre>",counter) == counter && state == TokenState.NONE) {
				...
				if(str.substring(0,counter).equals("</pre>"))
					state = TokenState.NONE;
			}
			else if (symbol == '<' && state == TokenState.NONE) {
			...
			else if (isPunctuation(symbol) && state == TokenState.NONE) {
				// if there is already a token, end it as long as it is
				// length > 0
				if (token.length() > 0) {
					tokenNum++;
			...
			else if (!Character.isWhitespace(symbol) && state == TokenState.NONE) {
				token += symbol;
			}
