...
public class WorkingMVCipher {
	...
	public static void main(String[] args) {
		...
		WorkingMVCipher wmvc = new WorkingMVCipher();
		wmvc.run();
	}
		...
		do {
			...
			for (int a = 0; a <= originalInput.length() - 1; a++) {
				...
				if(isInputLetters(originalInput, oneInputCharacter)) {
					...
					// keyArray[countKeyIndex] = key.charAt(countKeyIndex % key.length());
					...
					if (isEncrypt) {
						...
						writer.write(encryptEachChar(updatedInput, oneInputCharacter));
					}
					else {
						...
						writer.write(decryptEachChar(updatedInput, oneInputCharacter));
