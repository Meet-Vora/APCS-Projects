...
public class ExprUtils {
	...
	public List<String> toPostfix(String expr) {
		...
		// List of tokenized expression
		List<String> listEx = tokenizeExpression(expr);
		
		for(int a = 0; a < listEx.size(); a++) {
			String symbol = listEx.get(a);
			if(!isOperator(symbol))
				result.add(symbol);
			// if at end of arraylist and operators still left in stack,
			// push all of them onto the result arraylist
			else {
				...
				else if(hasPrecedence(symbol, operatorStack.peek())) {
	...
	/*************************************************************************/
	/**
	 *	Evaluate infix expression and determine if it is valid (extra credit)
	 *	@param tokens	a List of String tokens making up an infix arithmetic expression
